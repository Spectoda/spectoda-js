...
export class SpectodaRuntime {
  ...
  autoSelect(
    ...
    logging.verbose(
      `autoSelect(criteria=${JSON.stringify(criteria)}, scan_period=${scan_period}, timeout=${timeout}`,
    )
    ...
    for (const previewController of Object.values(this.previewControllers)) {
      for (let portTag = A_ASCII_CODE; portTag <= D_ASCII_CODE; portTag++) {
        ...
        if (error_code === 0) {
          ...
          console.log(
            `Controller ${previewController.label}, Port ${String.fromCharCode(portTag)}:`,
            bitset.toString(),
          )
