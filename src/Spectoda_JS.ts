...
export class Spectoda_JS {
  ...
  construct(controller_config: object, constroller_mac_address: string) {
    logging.debug(
      `Spectoda_JS::construct(controller_config=${JSON.stringify(controller_config)}, constroller_mac_address=${constroller_mac_address})`,
    )
      ...
      const WasmInterfaceImplementation: Spectoda_WASMImplementation = {
        ...
        _onTnglUpdate: (tngl_bytes_vector, used_ids_vector) => {
          ...
          try {
            ...
            this.#runtimeReference.emit('tngl_update', {
        ...
        _onSynchronize: (synchronization: Synchronization) => {
          ...
          try {
            ...
            logging.debug(
              `🕒 $${this.#spectoda_wasm?.getLabel()}: ${synchronization.clock_timestamp}`,
            )
